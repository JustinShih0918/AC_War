...
#include "StoneTitan_Bullet.hpp"
#include "Scene/MainPlayScene.hpp"
#include "Engine/Point.hpp"


StoneTitan_Bullet::StoneTitan_Bullet(Engine::Point position, Engine::Point forwardDirection, float rotation, Character* parent, Character* target) :
...
void StoneTitan_Bullet::Update(float deltaTime) {
	...
	if (!Target) {
		...
		else std::cout << "TitanBullet Update error\n";
		if (!character) {
...
void StoneTitan_Bullet::OnExplode(Character* character) {
	Target->lockedBullets.erase(lockedBulletIterator);
