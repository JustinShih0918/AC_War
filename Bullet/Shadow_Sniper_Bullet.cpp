...
#include "Shadow_Sniper_Bullet.hpp"
...
Shadow_Sniper_Bullet::Shadow_Sniper_Bullet(Engine::Point position, Engine::Point forwardDirection, float rotation, Character* parent, Character* target) :
	Bullet("Mainplay/Shadow_Sniper_Bullet.png", 300, 30, position, forwardDirection, rotation + ALLEGRO_PI / 2, parent, target) {
		if(!Target)
...
void Shadow_Sniper_Bullet::Update(float deltaTime) {
	...
	if (!Target) {
		...
		else std::cout << "ShadowBullet Update error\n";
		if (!character) {
...
void Shadow_Sniper_Bullet::OnExplode(Character* character) {
	Target->lockedBullets.erase(lockedBulletIterator);
