...
MainTower_Bullet::MainTower_Bullet(Engine::Point position, Engine::Point forwardDirection, float rotation, Character* parent, Character* target, std::string img) :
	Bullet(img, 300, 120, position, forwardDirection, rotation + ALLEGRO_PI / 2, parent, target) {
		if(!Target)
...
void MainTower_Bullet::Update(float deltaTime) {
	...
	if (!Target) {
		...
		else std::cout << "MainTowerBullet Update error\n";
		if (!character) {
...
void MainTower_Bullet::OnExplode(Character* character) {
	Target->lockedBullets.erase(lockedBulletIterator);
